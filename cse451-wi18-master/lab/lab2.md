...
From lab2 on, we will be asking you to write a small design document. You will turn it
in on Canvas. See the 451 Canvas for the due date. Follow the guidelines in
[how to write a design document](designdoc.md).
...
`sleep` is the equivalent of `cv::wait`; it puts the thread to sleep and releases
...
### Exercise
Fix your lab 1 code to be safe for concurrency.  In particular,
...
```c
/*
 * arg0: integer value signalling result status, generally negative if program
...
remaining buffered data, and then an error.
...
$ make turnin
```
This will produce a `tar.gz` archive with your code. Upload this to [the lab 2 dropbox](https://canvas.uw.edu/courses/1124225/assignments/4065377).
