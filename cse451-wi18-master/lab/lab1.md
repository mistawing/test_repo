...
```
$ git clone git@gitlab.cs.washington.edu:xk-public/18wi.git xk
Cloning into xk...
$ cd xk
```
...
as `Reporter` in your repository, and add your team members as a Master.
...
$ git remote add upstream git@gitlab.cs.washington.edu:xk-public/18wi.git
```
...
### Question #5
What is the first line of code executed in the kernel when there is an interrupt?

### Question #6
How large (in bytes) is a trap frame?

### Question #7
Set a breakpoint in the kernel implementation of a system call (e.g., `sys_write`) and continue
executing until the breakpoint is hit. Do a backtrace. What kernel functions are reported by
the backtrace when it reaches `sys_write`?
...
- A simple inode layer is already implemented. You can use readi/writei to read/write from an inode. You can use namei to find and load an inode. When no process is using an inode, inode should be released by irelease.
 ...
 *
 * Fewer than arg0 bytes can be read in various conditions:
 * If the current position + arg0 is beyond the end of the file.
 * If this is a pipe or console device and fewer than arg0 bytes are available 
...
$ make turnin
```
This will produce a `tar.gz` archive with your code. Upload this to [the lab 1 dropbox](https://canvas.uw.edu/courses/1124225/assignments/4062298).
