...
(i.e., `vregion[MR_HEAP].size = 0`), and so the first call to `malloc`
always calls `sbrk`.

xk has to track how much memory has been allocated to each processes' heap.
xk also has to allocate memory and free memory pages (using `kalloc` and `kfree`) on
behalf of the user. Remember, xk allocates and frees memory at page granularity
(i.e., 4096 bytes) but `sbrk` need to support allocating/deallocating memory at
byte-level. The OS does this to be portable, since an application can't depend
on the machine adopting a specific page size.

After the kernel allocates the memory on behalf of user, the kernel has to
map those memory into user's address space (using `vregionaddmap` in `kernel/vspace.c`)
...
```c
/*
 * arg0: integer value of amount of memory to be added to the heap
 *
 * Adds arg0 to the current heap; returns the previous heap limit, or -1
 * on error.
 *
 * Error condition:
 ...
 * -1 should still be returned, and nothing added to the heap.
 */
int
sys_sbrk(void);
```
...
### Question #2:
Explain how the file descriptors of `ls` and `wc` change when user types `ls | wc`
in xk's shell.
...
For every page in a process' page table (vspace) there is a structure that
...
From here you can allocate a page and copy the data from the copy-on-write page,
and let the faulting process start writing to that page.
...
When you're finished, create a `end_lab3` git tag as described above so we know the point at which you
submitted your code. In the root xk directory, run the turnin script:
```
$ make turnin
```
This will produce a `tar.gz` archive with your code. Upload this to [the lab 3 dropbox](
https://canvas.uw.edu/courses/1124225/assignments/4069555).